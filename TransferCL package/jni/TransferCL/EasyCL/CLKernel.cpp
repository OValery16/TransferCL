...
size_t CLKernel::get_kernel_work_group_size(){
	//olivier: add support for poor mobile GPU
	size_t workgroup_size=0;
	int  err=clGetKernelWorkGroupInfo(kernel,easycl->device,CL_KERNEL_WORK_GROUP_SIZE,sizeof(size_t), &workgroup_size, NULL);
	if (err != CL_SUCCESS) {
		//LOGI("Unable to get kernel work-group size");
	}
	size_t temp;
	err=clGetKernelWorkGroupInfo(kernel,easycl->device,CL_KERNEL_LOCAL_MEM_SIZE,sizeof(size_t), &temp, NULL);
	//LOGI("CL_KERNEL_LOCAL_MEM_SIZE %zu",temp);
	err=clGetKernelWorkGroupInfo(kernel,easycl->device,CL_DEVICE_LOCAL_MEM_SIZE,sizeof(size_t), &temp, NULL);
	//LOGI("CL_DEVICE_LOCAL_MEM_SIZE %zu",temp);
...
//  }
  ...
  if(error != 0) {
      ...
      for(int i = 0; i < (int)splitSource.size(); i++) {
	    ...
	    switch (error) {
	        case -4:
	        	LOGI("Memory object allocation failure, code -4");
		        throw std::runtime_error(sourceWithNumbers + "\nMemory object allocation failure, code -4");
		        break;
	        case -5:
	        	LOGI("Out of resources, code -5");
		        throw std::runtime_error(sourceWithNumbers + "\nOut of resources, code -5");
		        break;
	        case -11:
	        	LOGI("Program build failure, code -11");
		        throw std::runtime_error(sourceWithNumbers + "\nProgram build failure, code -11");
		        break;
	        case -46:
	        	LOGI("Invalid kernel name, code -46");
		        throw std::runtime_error(sourceWithNumbers + "\nInvalid kernel name, code -46");
		        break;
	        case -52:
	        	LOGI("Invalid kernel args, code -52");
		        throw std::runtime_error(sourceWithNumbers + "\nInvalid kernel args, code -52");
		        break;
	        case -54:
	        	LOGI("Invalid work group size, code -54");
		        throw std::runtime_error(sourceWithNumbers + "\nInvalid work group size, code -54");
		        break;
	        default:
	        	LOGI("Something went wrong, code %d",error);
